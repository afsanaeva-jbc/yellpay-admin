...
import type { Grade, Student } from "../../models/models";
...
const CreateStudents: React.FC = () => {
  const {t} = useTranslation(["viewDetails", "common", "student"])
  const { grades } = useGradesList();
  const { register, handleSubmit, formState, setValue } = useForm<Student>({
    mode: "onChange",
  });
  ...
  const onSubmit = async (data: Student) => {
    ...
    const plainGuardianContact = data?.guardian_contact?.replace(/\D/g, "");

    const studentPayload = {
      ...data,
      phone: plainPhone,
      guardian_contact: plainGuardianContact,
    };
    setLoading(true);

    try {
      const response = await axiosInstance.post("/students", studentPayload);
            ...
            {localGrades.length > 0 ? (
              <form onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                <div>
                  <div className="mb-2">
                    <label
                      className={`text-base ${errors.grade?.id ? "text-red-500" : "text-[#D5242A]"} font-bold`}
                    >
                      {t("class")}
                    </label>
                  </div>
                  <select
                    ...
                    {...register("grade_id", {
                      ...
                      valueAsNumber: true, // Ensure it's submitted as number
                    })}
