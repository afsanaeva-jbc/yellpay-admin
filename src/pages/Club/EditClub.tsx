...
interface EditClubProps {
  ...
  clubData?: Club | null | undefined;
}
const EditClub: React.FC<EditClubProps> = ({ clubData, clubId }) => {
  ...
  useEffect(() => {
    if (clubData) {
      const AdvisorOption = {
        value: clubData.advisor?.id?.toString() || "",
        label: `${clubData.advisor?.name} - "Advisor"`,
      };
      const memberOption: { [id: string]: boolean } = {};
      clubData.members?.forEach((member) => {
        memberOption[member.id.toString()] = true;
      });
      const subAdvisorMap: { [id: string]: boolean } = {};
      clubData.subadvisors?.forEach((value) => {
        subAdvisorMap[value.id.toString()] = true;
      });
