...
import DynamicForm, { FormConfig, FieldConfig } from '../../components/common/DynamicForm';
...
const CreateMerchantForm: React.FC = () => {
  ...
  const merchantValidationSchema = Yup.object().shape({
    ...
    logo: Yup.mixed<FileList>()
      .test('fileRequired', 'Logo is required for active merchants', function (value) {
        if (this.parent.status === 'active' && (!value || (value as FileList).length === 0)) {
          return false;
        }
        return true;
      })
  ...
  const createMerchantConfig: FormConfig = {
    ...
    onCancel: () => {
      const hasChanges = window.confirm('Are you sure you want to cancel? All unsaved changes will be lost.');
      if (hasChanges) {
        navigate(-1);
      }
    },
